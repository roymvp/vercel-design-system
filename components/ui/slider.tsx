...
*/
function Slider({ className, 'aria-label': ariaLabel, ...props }: React.ComponentProps<typeof BaseSlider.Root>) {
  return (
    <BaseSlider.Root data-slot="slider" className={cn('w-full', className)} {...props}>
      <BaseSlider.Control className="flex w-full items-center py-2">
        <BaseSlider.Track className="h-1 w-full rounded-[var(--radius-full)] bg-canvas-soft">
          ...
          <BaseSlider.Thumb aria-label={ariaLabel} className="size-4 rounded-[var(--radius-full)] border border-hairline bg-canvas shadow-[var(--shadow-xs)] outline-none focus-visible:ring-2 focus-visible:ring-ring/40" />
        </BaseSlider.Track>
      </BaseSlider.Control>
    </BaseSlider.Root>
  )
